...
public class JwtUtil {
    ...
    public String getUserIdFromHeaders(String authToken, String userIdHeader) {
        ...
        if (userIdHeader != null && !userIdHeader.isEmpty()) {
            // If X-User-Id is a username, map it to numeric ID
            if ("jane123".equals(userIdHeader)) {
                return "42";
            }
            return userIdHeader;
        }
        ...
        if (authToken != null && validateToken(authToken)) {
            ...
            // Map username to numeric user ID
            if ("jane123".equals(username)) {
                return "42"; // Map jane123 to user ID 42
            }
            // For other usernames, you might want to query a user service
            // For now, return a default or handle accordingly
